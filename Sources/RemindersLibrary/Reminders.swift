...
public final class Reminders {
        ...
        self.reminders(on: self.getCalendars(), displayOptions: displayOptions) { reminders in
            ...
            for (i, reminder) in reminders.enumerated() {
                ...
                let sameDay = calendar.compare(
                    reminderDueDate, to: dueDate, toGranularity: Calendar.Component.day) == .orderedSame
                let earlierDay = calendar.compare(
                    reminderDueDate, to: dueDate, toGranularity: Calendar.Component.day) == .orderedAscending
    ...
    func showListItems(withNameOrId nameOrId: String, dueOn dueDate: DateComponents?, includeOverdue: Bool,
        displayOptions: DisplayOptions, outputFormat: OutputFormat, sort: Sort, sortOrder: CustomSortOrder)
    {
        ...
        self.reminders(on: [reminderCalendar], displayOptions: displayOptions) { reminders in
            ...
            for (i, reminder) in reminders.enumerated() {
                ...
                let sameDay = currentCalendar.compare(
                    reminderDueDate, to: dueDate, toGranularity: Calendar.Component.day) == .orderedSame
                let earlierDay = currentCalendar.compare(
                    reminderDueDate, to: dueDate, toGranularity: Calendar.Component.day) == .orderedAscending
        ...
        outputFormat: OutputFormat)
    {
        ...
        if let dueDate = dueDate, dueDate.hour != nil {
            if let absoluteDate = dueDate.date {
                reminder.addAlarm(EKAlarm(absoluteDate: absoluteDate))
            }
        }
